...
\begin{itemize}[leftmargin=*]
  \item \textbf{Backend} (\repobackend) --- ocho servicios contenedorizados: \texttt{gateway} (NestJS), \texttt{realtime} (Socket.io), \texttt{optimizer} (gRPC + VROOM), \texttt{ai-predictor} (Python/Flask), \texttt{vroom} (motor VRP), \texttt{n8n} (automatizaci\'on), \texttt{openclaw} (notificaciones multicanal), \texttt{postgres} (base de datos) y \texttt{redis} (cach\'e y broker pub/sub).
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Microservicios} --- 8 servicios contenedorizados independientes, cada uno con responsabilidad \'unica, ciclo de vida propio y posibilidad de despliegue independiente.
...
\begin{lstlisting}[language=yaml,caption={Workflow CI del backend (.github/workflows/ci.yml)}]
strategy:
  matrix:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Stack distribuido cloud-native} con 8 servicios contenedorizados, IaC completa con Terraform, CI/CD autom\'atico v\'ia GitHub Actions.
